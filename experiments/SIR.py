...
def numpyro_SIR(likelihood, obs=None):
    R0 = numpyro.sample("R0", dist.Uniform(0.1, 10))
    gamma_inv = numpyro.sample("gamma_inv", TruncatedGamma(10, 2, low=1.0))
    numpyro.sample("fade_out_initial", dist.Bernoulli(1 / (R0 + 1)), obs=0)
    params = stack_and_broadcast(
        R0 * jnp.ones(1), gamma_inv * jnp.ones(1), broadcast_to=obs.data
    )
    context = Context(
        params=params,
...
def run_snl(args, snl_object, snl_config, dataset, logger):
    ...
    rng = jax.random.PRNGKey(hash("SIR" + label))
    t0 = time.time()
    snl_object.SNL(
        rng,
        config=snl_config,
...
def main(args):
    assert args.pop_size in [50, 100, 200]
    make_folder("SIR_experiments")
...
Experiment details:
    ...
    prior samples: {args.init_samples}
        ...
        with open(f"data/SIR_{args.pop_size}_pop_data.pkl", "rb") as f:
            obs = dill.load(f)

        if args.pop_size == 50:
            tr = 50
        elif args.pop_size == 100:
            tr = 60
        else:
            tr = 100
        ...
        def simulate(R0, gamma_inv):
            ...
            Z1, Z2 = SIR(
                N=args.pop_size * np.ones(R0.shape[0]),
                initial_infected=1,
                time_run=tr,
                beta=beta.reshape(-1),
            ...
            params = np.stack((R0, gamma_inv), axis=-1)
            return Dataset(
                data=Z1,
                context=Context(
                    params=params,
